...
export const listProperties = async (req: Request, res: Response) => {
  try {
    ...
    if (city) filters['location.city'] = { $regex: new RegExp(city as string, 'i') };
    if (minPrice || maxPrice)
      filters.price = {
        ...(minPrice ? { $gte: Number(minPrice) } : {}),
        ...(maxPrice ? { $lte: Number(maxPrice) } : {})
      };
    if (beds) filters.bedrooms = { $gte: Number(beds) };
    if (baths) filters.bathrooms = { $gte: Number(baths) };
...
export const updateProperty = async (req: Request, res: Response) => {
  try {
    ...
    const property = await Property.findOneAndUpdate(
      { _id: req.params.id, agentId: user.sub },
      req.body,
      { new: true }
    );
